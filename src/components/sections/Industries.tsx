          ...
          <ul role="tablist" aria-label={dict.industries.title} className="mt-12 flex flex-wrap gap-x-5 gap-y-1 sm:gap-x-8">
            {industries.map((item) => (
              <li key={item.id}>
                ...
                >
                  {item.label[locale]}
                  {selected === item.id && <motion.span layoutId="industry-underline" className="absolute inset-x-0 bottom-1 h-[2px] bg-accent" transition={{ type: 'spring', stiffness: 380, damping: 34 }} />}
                </button>
              </li>
            ))}
          </ul>
                    ...
                    {related.map((p) => (
                      <li key={p.slug}>
                        <Link href={href(locale, `/work/${p.slug}`)} data-cursor="view" className="group flex items-center gap-4 rounded-2xl border border-line p-3 transition-colors hover:border-paper/25">
                          ...
                          <ArrowUpRight className="size-4 text-mute transition-colors group-hover:text-accent rtl:-scale-x-100" strokeWidth={1.6} />
                        </Link>
                      </li>
                    ))}
                  </ul>
