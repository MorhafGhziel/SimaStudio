          ...
          <ul role="tablist" aria-label={dict.industries.title} className="mt-12 flex flex-wrap gap-x-5 gap-y-1 sm:gap-x-8">
            {industries.map((item) => (
              <li key={item.id}>
                ...
                >
                  {item.label[locale]}
                  {selected === item.id && <motion.span layoutId="industry-underline" className="absolute inset-x-0 bottom-1 h-[2px] bg-linear-to-r from-violet to-blue rtl:bg-linear-to-l" transition={{ type: 'spring', stiffness: 380, damping: 34 }} />}
                </button>
              </li>
            ))}
          </ul>
                    ...
                    {related.map((p) => (
                      <li key={p.slug}>
                        <Link href={href(locale, `/work/${p.slug}`)} data-cursor="view" className="group flex items-center gap-4 rounded-2xl border border-line p-3 transition-colors hover:border-paper/25">
                          ...
                          <ArrowUpRight className="size-4 text-mute transition-colors group-hover:text-violet-soft rtl:-scale-x-100" strokeWidth={1.6} />
                        </Link>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <Link href={`${href(locale)}#contact`} className="group inline-flex items-center gap-2 text-paper/80 transition-colors hover:text-violet-soft">
