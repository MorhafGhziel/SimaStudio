...
export async function getDashboard(rangeKey: string | undefined) {
  ...
  const kpi = (a: string, b: string) => sql`
    ...
    from analytics_sessions where started_at >= ${a}::timestamptz and started_at < ${b}::timestamptz`;
  ...
  const breakdown = (column: keyof typeof BREAKDOWN_COLUMNS, limit = 12) =>
    sql.query(
      `select coalesce(${BREAKDOWN_COLUMNS[column]}, 'Unknown') as label, count(*)::int as sessions, count(distinct visitor_id)::int as visitors
       from analytics_sessions where started_at >= $1::timestamptz and started_at < $2::timestamptz
  ...
  const [
    ...
    sql`
      ...
      from analytics_events where type = 'pageview' and ts >= ${from}::timestamptz and ts < ${to}::timestamptz
      group by path order by views desc limit 15`,
    sql`select count(distinct visitor_id)::int as active from analytics_sessions where last_seen > now() - interval '5 minutes'`,
    sql`
      select exit_path as path, country, city, device, browser, os, source, pageviews,
    ...
    sql`
      ...
      from analytics_events where type = 'event' and ts >= ${from}::timestamptz and ts < ${to}::timestamptz
      group by name order by count desc`,
    sql`
      select props->>'section' as section, count(distinct session_id)::int as sessions
      from analytics_events where type = 'event' and name = 'section_view' and ts >= ${from}::timestamptz and ts < ${to}::timestamptz
      group by 1 order by 2 desc`,
    sql`
      select (props->>'depth')::int as depth, count(distinct session_id)::int as sessions
      from analytics_events where type = 'event' and name = 'scroll' and ts >= ${from}::timestamptz and ts < ${to}::timestamptz
    ...
    sql`
      ...
      where started_at >= ${from}::timestamptz and started_at < ${to}::timestamptz and latitude is not null and longitude is not null
    ...
    sql`
      select id, started_at, last_seen, entry_path, exit_path, pageviews, source, referrer_host, utm_campaign,
             country, region, city, device, browser, os, screen, language, locale, is_new,
