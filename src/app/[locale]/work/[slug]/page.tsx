...
export async function generateMetadata({ params }: PageProps<'/[locale]/work/[slug]'>): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/${locale}/work/${slug}`, languages: { en: `/en/work/${slug}`, ar: `/ar/work/${slug}` } },
      ...
      <Link href={href(locale, `/work/${next.slug}`)} data-cursor="view" className="group mt-28 block border-t border-line sm:mt-36">
        <div className="container-x grid items-center gap-8 py-16 md:grid-cols-12 md:py-24">
          <div className="md:col-span-6">
            ...
            <p className="display-lg mt-4 transition-colors duration-500 group-hover:text-silver">{locale === 'ar' ? next.arName : next.name}</p>
            <p className="mt-3 text-mute">{next.kind[locale]}</p>
            <ArrowUpRight className="mt-8 size-8 text-mute transition-transform duration-700 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-silver rtl:-scale-x-100" strokeWidth={1.2} />
          </div>
